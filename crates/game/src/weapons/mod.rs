...
use crate::{character::{movement::SprintState, player::{input::{CursorPosition, INPUT_RELOAD, INPUT_SWITCH_WEAPON_MODE}, jjrs::PeerConfig, Player}}, collider::{is_colliding, Collider, ColliderShape, CollisionLayer, CollisionSettings, Wall}, frame::FrameCount, global_asset::GlobalAsset};
...
pub fn weapon_rollback_system(
    ...
    mut inventory_query: Query<(Entity, &mut WeaponInventory, &SprintState, &CollisionLayer, &Player)>,
    ...
    for (entity,  mut inventory, sprint_state , collision_layer, player) in inventory_query.iter_mut() {
