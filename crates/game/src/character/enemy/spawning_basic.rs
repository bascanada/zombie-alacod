...
use crate::collider::{Collider, ColliderShape, is_colliding};
...
impl EnemySpawnSystem for BasicEnemySpawnSystem {
    fn should_spawn(&self, frame: &FrameCount, state: &EnemySpawnState, current_enemies: u32) -> bool {
        current_enemies < self.config.max_enemies && (frame.frame >= state.next_spawn_frame && frame.frame >= (state.last_spawn_frame + 20))
    }
    ...
    fn calculate_spawn_position(
        ...
        let player_count = player_positions.len();
        let mut player_positions = player_positions.to_vec();
        player_positions.sort_by(|a, b| {
            // Sort by x first, then by y for stability
            a.x.partial_cmp(&b.x)
